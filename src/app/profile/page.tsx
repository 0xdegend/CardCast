...
export default function ProfilePage() {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 text-center px-4">
        ...
        <p className="text-(--text-muted) max-w-sm">
          Your positions, XP, and achievements are stored onchain. Connect your
          wallet to access your dashboard.
        </p>
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
      {/* ── Profile Header ── */}
      <div className="relative overflow-hidden bg-(--bg-card) border border-(--border) rounded-2xl p-6 sm:p-8 mb-6">
        {/* Glow */}
        <div
        ...
        <div className="relative flex items-start gap-5 flex-wrap">
          {/* Avatar */}
          <div className="w-16 h-16 rounded-full bg-linear-to-br from-(--teal) to-(--purple) flex items-center justify-center text-xl font-extrabold text-black border-2 border-[rgba(0,229,204,0.3)] shrink-0">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              {user.streak > 0 && (
                <Badge variant="gold">🔥 {user.streak} Streak</Badge>
              )}
            </div>
            ...
            <div className="flex gap-6 flex-wrap">
              {[
                ...
                {
                  label: "Total PnL",
                  value: formatUSD(user.totalPnl),
                  color: "var(--teal)",
                },
                {
                  label: "Markets",
                  value: String(user.marketsTraded),
                  color: "var(--purple)",
                },
          ...
          <div className="flex flex-col gap-3 min-w-45">
            <div className="bg-white/3 border border-(--border) rounded-md p-3.5">
              <p className="text-[10px] uppercase tracking-widest text-(--text-dim) mb-2 font-display">
                XP to Next Rank
              </p>
              <ProgressBar
                value={Math.round((user.xp / 33000) * 100)}
                color="gold"
              />
              <p className="font-mono text-[11px] text-(--text-dim) mt-1.5">
                {formatXP(user.xp)} / 33,000
              </p>
            </div>
        ...
        {/* Badges */}
        <div className="relative flex items-center gap-2 mt-5 pt-5 border-t border-(--border) flex-wrap">
          {user.badges.map((b) => (
            <div
              key={b.id}
              className={cn(
                ...
                b.rarity === "legendary"
                  ? "bg-[rgba(240,180,41,0.1)] border-[rgba(240,180,41,0.3)] text-(--gold)"
                  : b.rarity === "epic"
                    ? "bg-[rgba(155,109,255,0.1)] border-[rgba(155,109,255,0.3)] text-(--purple)"
                    : b.rarity === "rare"
                      ? "bg-[rgba(0,229,204,0.1)] border-[rgba(0,229,204,0.3)] text-(--teal)"
      ...
      <div className="flex gap-1.5 mb-5">
        {(["Positions", "History", "Achievements", "Watchlist"] as Tab[]).map(
          (t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={cn(
                "px-4 py-2 rounded-sm text-xs font-bold uppercase tracking-wider transition-all font-display",
                tab === t
                  ? "bg-[rgba(0,229,204,0.1)] text-(--teal) border border-[rgba(0,229,204,0.2)]"
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              <div
                key={label}
                className="bg-(--bg-card) border border-[var(--border) rounded-md p-4"
              >
                <p className="text-[10px] uppercase tracking-widest text-(--text-dim) mb-1.5 font-display">
                  {label}
                </p>
          ...
          {POSITIONS.map((p) => (
            <Link key={p.id} href={`/market/${p.marketId}`}>
              <div className="bg-(--bg-card) border border-(--border) rounded-lg p-4 card-hover flex items-center gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1.5">
                    <Badge variant={p.outcome === "yes" ? "teal" : "red"}>
                  ...
                  <div className="flex items-center gap-3 text-xs text-(--text-dim) font-mono">
                    ...
                    <span>
                      Entry {Math.round(p.entryPrice * 100)}% → Now{" "}
                ...
                <div className="text-right shrink-0">
                  ...
                  <p className="text-[10px] text-(--text-dim) mt-0.5 uppercase tracking-wider font-display">
                    {p.status}
                  </p>
                </div>
              </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {ACHIEVEMENTS.map((a) => (
            <div
              key={a.id}
              className={cn(
                "bg-(--bg-card) border rounded-lg p-4 text-center transition-all",
                a.unlocked
          ...
          {watchlistMarkets.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="font-display font-bold text-lg mb-2">
                No markets bookmarked
              </p>
              <p className="text-sm text-(--text-muted) mb-4">
                Save markets to track them here.
              </p>
              <Link href="/markets">
